<template>
  <section class="services pages__rectangle-bg">
    ...
    <div class="container">
      ...
      <div
        data-aos="fade-right"
        class="services__subtitle pages__subtitle"
        v-html="getPropertyLang(servicesSubtitle)"
      >
      </div>
      ...
      <div class="services__block">
        <div class="row gy-5 g-sm-5">
          <div
            class="col-lg-4 col-sm-6 col-10 offset-1 offset-sm-0"
          ...
          >
            <NuxtLink
              ...
              class="services__block-btn"
            >
              <div class="services__block-btn-img">
...
export default {
  ...
  async asyncData({$axios}) {
    ...
    await $axios('website/section/services')
      .then((response) => {
        servicesSeo = response.data.seo_meta_as_array.head
    ...
    return {servicesSeo, servicesSubtitle, topImage} //services, serviceId, serviceData
  },

  head() {
    let seo = this.servicesSeo[this.currentLang]
    ...
    return {
      title: `ARDENA - ${this.$t('titles.services')}`,
      link: [{
  ...
  methods: {
    async getAllServices() {
      await this.$axios.get('website/company/service')
        .then(res => {
          this.services = res.data.data.reverse()
        })
        .catch(err => {
          console.error(err)
        })
